...
.topbar{position:fixed;top:0;left:0;right:0;height:52px;background:#1a1a1a;display:flex;align-items:center;padding:0 16px;z-index:100;border-bottom:1px solid #2a2a2a}
.topbar h1{font-size:1.1rem;font-weight:600;color:#ddd;flex:1;margin-left:12px}
.hbtn{background:none;border:none;cursor:pointer;padding:6px;display:flex;flex-direction:column;gap:5px}
.hbtn span{display:block;width:22px;height:2px;background:#ccc;border-radius:2px;transition:.25s}
...
.overlay{display:none;position:fixed;inset:0;top:52px;background:rgba(0,0,0,.5);z-index:80}
.overlay.show{display:block}
.content{margin-top:52px;padding:16px;max-width:480px;margin-left:auto;margin-right:auto}
.view{display:none}.view.active{display:block}
.card{background:#1e1e1e;border-radius:12px;padding:16px;margin-bottom:14px}
.card h2{font-size:.75rem;color:#555;text-transform:uppercase;letter-spacing:.07em;margin-bottom:14px}
.co2{font-size:3.2rem;font-weight:700;text-align:center;line-height:1}
.co2-unit{text-align:center;color:#444;font-size:.8rem;margin:.4rem 0 1rem}
.g{color:#4caf50}.y{color:#ffc107}.r{color:#f44336}.x{color:#444}
.grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}
.tile{background:#272727;border-radius:8px;padding:10px 12px}
.tile-val{font-size:1.2rem;font-weight:600}
.tile-lbl{font-size:.68rem;color:#555;margin-top:3px}
...
.fan-off{background:#222;color:#555}
.fan-low{background:#152240;color:#5599ee}
.fan-high{background:#301515;color:#ee5555}
.cooldown{text-align:center;color:#ffc107;font-size:.88rem;min-height:1.4em}
...
.btn-cd{background:#1565c0;color:#fff}
.btn-off{background:#b71c1c;color:#fff}
.mtabs{display:flex;gap:6px;margin-bottom:12px;flex-wrap:wrap}
.mtab{padding:6px 12px;border:1px solid #333;border-radius:6px;background:none;color:#888;font-size:.8rem;cursor:pointer;transition:.15s}
.mtab.active{background:#5599ee22;border-color:#5599ee;color:#5599ee}
canvas{display:block;width:100%;border-radius:8px}
.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-top:12px}
.stat{background:#272727;border-radius:8px;padding:8px;text-align:center}
.stat-val{font-size:1rem;font-weight:600}
.stat-lbl{font-size:.65rem;color:#555;margin-top:2px}
.sbar{display:flex;justify-content:space-between;font-size:.7rem;color:#3a3a3a;margin-top:4px}
.err{background:#2a1010;border:1px solid #7b1c1c;border-radius:8px;padding:10px;color:#ef9a9a;text-align:center;margin-bottom:12px;display:none;font-size:.88rem}
.hint{color:#444;font-size:.8rem;text-align:center;padding:20px 0}
</style>
</head>
<body>
<div class="topbar">
  <button class="hbtn" id="hbtn" onclick="toggleDrawer()" aria-label="Menu">
...
<div class="content">
  ...
  <div id="view-controls" class="view">
    <div class="card">
      ...
      <div class="btns">
        <button class="btn btn-on" onclick="ctrl('on')">Turn On</button>
        <button class="btn btn-cd" onclick="ctrl('off_cooldown')">Turn Off (with cooldown)</button>
        <button class="btn btn-off" onclick="ctrl('off_immediate')">Turn Off Immediately</button>
      </div>
    </div>
  </div>
  <div id="view-history" class="view">
    <div class="card">
      <h2>Long-term trends <span style="font-weight:400;color:#444;text-transform:none;letter-spacing:0">&#8212; last 12 h</span></h2>
      <div class="mtabs">
...
<script>
function toggleDrawer(){const o=document.getElementById('drawer').classList.toggle('open');document.getElementById('hbtn').classList.toggle('open',o);document.getElementById('overlay').classList.toggle('show',o);}
function closeDrawer(){document.getElementById('drawer').classList.remove('open');document.getElementById('hbtn').classList.remove('open');document.getElementById('overlay').classList.remove('show');}
function nav(id,el){
  document.querySelectorAll('.view').forEach(v=>v.classList.remove('active'));
  document.getElementById('view-'+id).classList.add('active');
  document.querySelectorAll('.drawer a').forEach(a=>a.classList.remove('active'));
  el.classList.add('active');closeDrawer();
...
const METRICS={co2:{key:'co2',color:'#5599ee'},temp:{key:'temp',color:'#ff8c42'},hum:{key:'hum',color:'#42d7ff'},pm25:{key:'pm25',color:'#bb88ff'}};
...
function drawLineChart(canvas,data,m){
  ...
  [0,0.5,1].forEach(f=>{
    ...
    const lbl=dt<60?'now':dt<3600?'-'+Math.round(dt/60)+'m':'-'+Math.round(dt/360)/10+'h';
...
function drawFanChart(canvas,data){
  ...
  data.forEach((e,i)=>{if(e.fan){ctx.fillStyle='#ee555566';ctx.fillRect(i*bw,4,Math.max(bw,1),H-8);}});
  ctx.fillStyle='#444';ctx.font='9px sans-serif';ctx.textAlign='left';ctx.fillText('fan on',4,H-6);
}
...
static void handleControl() {
    ...
    if (action == "on") {
        g_state->pending_action = PendingAction::TURN_ON;
    } else if (action == "off_cooldown") {
        g_state->pending_action = PendingAction::OFF_COOLDOWN;
    } else if (action == "off_immediate") {
